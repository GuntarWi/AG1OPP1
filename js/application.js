...
*/
          ...
          if(data[i][6].formattedValue == "BANKER"){
            Banker += data[i][8].value;
          }else if (data[i][6].formattedValue == "PLAYER"){
            Player += data[i][8].value;
          }
        ...
        const rowObj = {
         ...
         //   UserName: data[i][4].value,
            TableType: data[i][4].formattedValue,
            TableId: data[i][5].formattedValue,
            BetPosition: data[i][6].formattedValue,
            Currency: data[i][7].formattedValue,
            Amount: data[i][8].value,
            Net: data[i][9].value,
           UniqueGroup: "no",
      ...
      if (data[i][2].formattedValue === "starfish") {
        rowObj.UniquePlayerID = data[i][3].value + "/3";
      } else if (data[i][2].formattedValue === "mayflypm") {
        rowObj.UniquePlayerID = data[i][3].value + "/4";
      } else if (data[i][2].formattedValue === "mayfly") {
        rowObj.UniquePlayerID = data[i][3].value + "/5";
      } else if (data[i][2].formattedValue === "mayflyby") {
        rowObj.UniquePlayerID = data[i][3].value + "/6";
      } else if (data[i][2].formattedValue === "mayflywh") {
        rowObj.UniquePlayerID = data[i][3].value + "/7";
      } else {
        //rowObj.UniquePlayerID = data[i][3].value + "/" + "NEW Opperator Conntact ME!";
        rowObj.UniquePlayerID = data[i][3].value + "/" + data[i][2].value;
      }
...
function GenerateHtmalTable(matchingRows){
  ...
  */
          const TableType = row.insertCell(4);
          TableType.innerHTML = data[i].TableType;
  
          const TableId = row.insertCell(5);
          TableId.innerHTML = data[i].TableId;
  
          const BetPosition = row.insertCell(6);
          BetPosition.innerHTML = data[i].BetPosition;
  
          const Currency = row.insertCell(7);
          Currency.innerHTML = data[i].Currency;
  
  
          const Amount = row.insertCell(8);
          Amount.innerHTML = data[i].Amount;
  
  
          const Net = row.insertCell(9);
